 ...
 * 问题原因：在操作LED的时候，正好碰到定时器中断，导致P0口数据不稳定，造成LED微亮
 * 解决方案：nixie.c中第8行改成 Y6;P0=0X01<<i;Y;							因为中断中不要频繁更改P0口数据
 *			define.h中第14行改成 #define LED_OFF Y4;P0=0XFF;Y//关闭led	最好先开锁存器，再操作数据
 * 			通过此方案可以改善LED闪烁、微亮的问题！
