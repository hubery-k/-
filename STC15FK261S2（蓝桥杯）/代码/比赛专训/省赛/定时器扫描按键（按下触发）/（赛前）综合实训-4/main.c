...
void Hardware_Init()
{
	...
	RB_Init();		//关闭蜂鸣器、继电器
	Timer0Init();	//定时器0初始化
	Uart_Init();		//串口（使用定时器2）初始化
	Sonic_Init();	//超声波（使用定时器1）初始化
	DS1302_WriteTime(Time);	//DS1302写入时间
...
void USE_PCF8591()
{
	...
	{
		AD_flag=0;
		adc_v=PCF8591_ReadAD(Channel)*(5.0/255);
		PCF8591_WriteDA(adc_v/5.0*255);
	}	
}
...
void USE_AT24C02()
{
	...
	{
		Write_flag=0;
		AT24C02_WriteByte(0X00,Num);	//写字节最大需要5ms时间
		Delay5ms();
	}
	if(Read_flag==1)
	{
		Read_flag=0;
		Num=AT24C02_ReadByte(0X00);
	}
}
...
void USE_UART()
{
	...
	if(Rec_flag==1)
	{
		...
		if(strcmp(Rec_Buf,"LED_Close")==0)
		{
			LED_Enable=0;
			Uart_SendString("LED_Close_Ok!\r\n");
		}
		else if(strcmp(Rec_Buf,"LED_Open")==0)
		{
			LED_Enable=1;
			Uart_SendString("LED_Open_Ok!\r\n");
		}
		else if(strcmp(Rec_Buf,"SMG_Open")==0)
		{
			SMG_Enable=1;
			Uart_SendString("SMG_Open_Ok!\r\n");
		}
		else if(strcmp(Rec_Buf,"SMG_Close")==0)
		{
			SMG_Enable=0;
			Uart_SendString("SMG_Close_Ok!\r\n");
		}
	}
}
...
void USE_KEY()
{
	...
	if(Cont&0X28){Press_flag=1;}
	else{Press_flag=0;}
	
	if(Press_State==1){T_p+=1;PressCount=0;Press_State=0;}//长按1s温度参数加1
}
...
void Timer0_Routine(void) interrupt 1
{
	...
	else{PressCount=0;Press_State=0;}
